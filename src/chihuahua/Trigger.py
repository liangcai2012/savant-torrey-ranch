...
def listQueueItems(args):
    request = {"mod":"view","cmd":"list"}
    send_request(request)

def moveQueueItem(args):
    try:
        index = args.index
...
def listRunningStrategy(args):
    request = {"mod":"str","cmd":"list"}
    send_request(request)

def stopStrategy(args):
    try:
        index = args.index
    except NameError:
        print "Incorrect arguments"
...
def main():
    ...
    psr_vi.set_defaults(func=viewSymbol)    

    psr_vl = subparsers.add_parser("vl",help="list all items in the message queue")
    psr_vl.set_defaults(func=listQueueItems)
    ...
    psr_vm.set_defaults(func=moveQueueItem)
    ...
    psr_st.set_defaults(func=stopStrategy)    
